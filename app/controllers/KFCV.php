...
class KFCV extends Controller{
    public function index(){
        ...
        // shuffle($case);
        // foreach ($case as $key => $cs) {
            //     echo '<pre>'; print_r($cs); echo '</pre>';
            //     $this->model('CBR_model')->WriteCase($cs);
            // }
        
        // RANGE TIAP FOLD ====================================
        // $case = $this->model('CBR_model')->getAllCaseKFCV();
        // echo count($case).'<br>';
        // $k_CV = 10;
        // $div = round(count($case)/$k_CV,0)+1;
        // $mod = $k_CV-(count($case)%$k_CV);
        // echo "$div - $mod<br>";
        // $minn = 0;
        // for ($i=0; $i < 10; $i++) { 
        //     $maxx = $minn+$div-1;
        ...
        // BUAT TABEL TESTING DAN TRAINING ====================
        $max = 375;
        $min = 338;
        $jumTest = 0;
        $case = $this->model('CBR_model')->getAllCaseKFCV();
        foreach ($case as $key => $cs) {
            if ($key<=$max && $key>=$min) {
                echo "$key <br>";
                $this->model('CBR_model')->WriteCaseSplit($cs);
                $jumTest++;
            }
        }
        foreach ($case as $key => $cs) {
    ...
    public function akurasi(){
        // AMBIL KASUS LAMA
        set_time_limit(500);
        $case = $this->model('CBR_model')->getAllCaseSplit();
